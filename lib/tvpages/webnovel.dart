...
import 'package:carousel_slider/carousel_slider.dart' show CarouselSlider, CarouselOptions, CarouselSliderController;
...
class WebNovelState extends State<WebNovel> {
  late NovelSectionDataProvider audioSectionProvider;
  ...
  Future<void> getTabData(
    ...
    await audioSectionProvider.getSectionBanner(
        ...
        position == 0 ? "1" : "2");
    await audioSectionProvider.getNovelSectionList(
        position == 0 ? "0" : (sectionTypeList?[position - 1].id),
        position == 0 ? "1" : "2",
        pageNo);
  }
            ...
            return Material(
              ...
              child: InkWell(
                ...
                onTap: () async {
                  ...
                  audioSectionProvider.sectionListData = [];
            ...
            children: [
              ...
              Consumer<NovelSectionDataProvider>(
                builder: (context, sectionDataProvider, child) {
                  if (sectionDataProvider.loadingBanner ?? false) {
                    ...
                    if ((sectionDataProvider.sectionBannerModel?.status ?? 0) == 200 &&
              ...
              Consumer<NovelSectionDataProvider>(
                builder: (context, sectionDataProvider, child) {
                  if ((sectionDataProvider.loadingSection ?? false) &&
                      (sectionDataProvider.loadmore ?? false) == false) {
                    return channelShimmer();
                  } else {
                    if ((sectionDataProvider.novelsectionListModel?.status ?? 0) ==
                        200) {
              ...
              Consumer<NovelSectionDataProvider>(
                builder: (context, sectionDataProvider, child) {
                  if (sectionDataProvider.loadmore ?? false) {
                    return Container(
                      height: 50,
  ...
  Widget setSectionByType(List<sectiondata.Result>? sectionList) {
    return ListView.separated(
  ...
  Widget setSectionData(
      {required List<sectiondata.Result>? sectionList, required int index}) {
    /* video_type =>  1-video,  2-show */
    /* screen_layout =>  landscape, potrait, square */
    if ((sectionList?[index].screenLayout ?? "") == "landscape") {
      return landscape(1, _convertToAudioDatum(sectionList?[index].data),
          sectionList?[index].screenLayout.toString() ?? "");
    } else if ((sectionList?[index].screenLayout ?? "") == "portrait") {
      return portrait(_convertToAudioDatum(sectionList?[index].data));
    } else if ((sectionList?[index].screenLayout ?? "") == "small_square") {
      return square(_convertToAudioDatum(sectionList?[index].data));
    } else if ((sectionList?[index].screenLayout ?? "") == "language") {
      return languageLayout(1, _convertToAudioDatum(sectionList?[index].data));
    } else if ((sectionList?[index].screenLayout ?? "") == "grid_view") {
      return miniSeries(1, _convertToAudioDatum(sectionList?[index].data));
    } else if ((sectionList?[index].screenLayout ?? "") == "big_square") {
      return bestSellingStories(1, _convertToAudioDatum(sectionList?[index].data));
    } else if ((sectionList?[index].screenLayout ?? "") == "list_view") {
      return top10series(1, _convertToAudioDatum(sectionList?[index].data));
    } else if ((sectionList?[index].screenLayout ?? "") == "details_square") {
      return newRelease(1, _convertToAudioDatum(sectionList?[index].data));
    } else if ((sectionList?[index].screenLayout ?? "") == "round") {
      return author(1, _convertToAudioDatum(sectionList?[index].data));
    } else if ((sectionList?[index].screenLayout ?? "") == "category") {
      return categories(1, _convertToAudioDatum(sectionList?[index].data));
    } else {
      return landscape(1, _convertToAudioDatum(sectionList?[index].data),
          sectionList?[index].screenLayout.toString() ?? "");
    }
  ...
  // Helper method to convert novel Datum to audio Datum
  List<list.Datum>? _convertToAudioDatum(List<sectiondata.Datum>? novelData) {
    if (novelData == null) return null;
    return novelData.map((novelDatum) => list.Datum(
      id: novelDatum.id,
      contentType: novelDatum.contentType,
      artistId: novelDatum.artistId,
      categoryId: novelDatum.categoryId,
      languageId: novelDatum.languageId,
      title: novelDatum.title,
      portraitImg: novelDatum.portraitImg,
      landscapeImg: novelDatum.landscapeImg,
      description: novelDatum.description,
      image: novelDatum.image,
