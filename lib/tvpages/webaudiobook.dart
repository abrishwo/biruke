...
class WebAudioBook extends StatefulWidget {
  final String? audioId, audioTitle, audioImage, audioFile, audioDuration;
  const WebAudioBook({
    super.key,
...
class _WebAudioBookState extends State<WebAudioBook>
  ...
  late AudioSectionDataProvider audioSectionDataProvider;
  ...
  @override
  void initState() {
    audioSectionDataProvider =
        Provider.of<AudioSectionDataProvider>(context, listen: false);
    profileProvider = Provider.of<ProfileProvider>(context, listen: false);
    showDetailsProvider = Provider.of<ShowDetailsProvider>(context, listen: false);
  ...
  _getData(pageno) async {
    ...
    await audioSectionDataProvider.setLoading(true);
  ...
  _scrollListener() async {
    ...
    if (_scrollController.offset >=
        ...
        (audioSectionDataProvider.currentPage ?? 0) <
            (audioSectionDataProvider.totalPage ?? 0)) {
      audioSectionDataProvider.setLoadMore(true);

      _fetchData(audioSectionDataProvider.currentPage ?? 0, 0);
    }
  }
  ...
  Future<void> getTabData(
    ...
    await audioSectionDataProvider.setLoading(true);
    await audioSectionDataProvider.getSectionBanner(
        position == 0 ? "0" : (sectionTypeList?[position - 1].id),
        position == 0 ? "1" : "2");
    await audioSectionDataProvider.getAudioSectionList(
        position == 0 ? "0" : (sectionTypeList?[position - 1].id),
        position == 0 ? "1" : "2",
        pageNo);
  }
  ...
  Future<void> setSelectedTab(int tabPos) async {
    ...
    debugPrint(
        "setSelectedTab lastTabPosition ====> ${audioSectionDataProvider.lastTabPosition}");
    if (audioSectionDataProvider.lastTabPosition == tabPos) {
      return;
    } else {
      audioSectionDataProvider.setTabPosition(tabPos);
    }
  }
            ...
            child: CarouselSlider.builder(
              ...
              options: CarouselOptions(
                ...
                onPageChanged: (val, _) async {
                  await audioSectionDataProvider.setCurrentBanner(val);
                },
              ),
              itemBuilder:
