...
class HomeState extends State<Home> {
  ...
  Widget setSectionByType(List<list.Result>? sectionList) {
    // Filter out thread and music sections
    List<list.Result>? filteredSectionList = sectionList?.where((section) {
      String sectionTitle = (section.title ?? "").toLowerCase();
      return !sectionTitle.contains('thread') && 
             !sectionTitle.contains('music') &&
             !sectionTitle.contains('podcast') &&
             !sectionTitle.contains('radio');
    }).toList();
    ...
    return ListView.separated(
      ...
      itemCount: filteredSectionList?.length ?? 0,
      shrinkWrap: true,
      physics: const NeverScrollableScrollPhysics(),
      itemBuilder: (BuildContext context, int index) {
        if (filteredSectionList?[index].data != null &&
            (filteredSectionList?[index].data?.length ?? 0) > 0) {
          return Container(
            padding: (filteredSectionList?[index].screenLayout ?? "") == "banner_view"
            ...
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                (filteredSectionList?[0].screenLayout ?? "") == "banner_view"
                    ? const SizedBox.shrink()
                    : index == 0
                        ? const SizedBox(
                            height: 25,
                        ...
                        : const SizedBox.shrink(),
                (filteredSectionList?[index].screenLayout ?? "") == "banner_view"
                    ? const SizedBox.shrink()
                    : Padding(
                          ...
                          children: [
                            Expanded(
                              child: MyText(
                                color: Colors.black87,
                                text:
                                    filteredSectionList?[index].title.toString() ?? "",
                                textalign: TextAlign.left,
                                fontsizeNormal: 14,
                                fontweight: FontWeight.w600,
                                fontsizeWeb: 16,
                            ...
                            filteredSectionList?[index].viewAll == 1
                                ? InkWell(
                                    onTap: () {
                                      Navigator.push(
                                          context,
                                          MaterialPageRoute(
                                              builder: (context) => ViewAll(
                                                    screenLayout:
                                                        filteredSectionList?[index]
                                                                .screenLayout
                                                                .toString() ??
                                                            "",
                                                    sectionId:
                                                        filteredSectionList?[index].id,
                                                    title: filteredSectionList?[index]
                                                            .title
                                                            .toString() ??
                                                        "",
                                                  )));
                ...
                SizedBox(
                    height: (filteredSectionList?[index].screenLayout ?? "") ==
                            "banner_view"
                        ? 0
                        : 5),
                setSectionData(sectionList: filteredSectionList, index: index),
              ],
            ),
          );
        } else {
